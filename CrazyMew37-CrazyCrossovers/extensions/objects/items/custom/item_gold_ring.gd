...
func on_money_changed(money: int) -> void:
	...
	if money_laff > 100 && EndlessNerfsSetting == 0:
		money_laff = 100
	Util.get_player().stats.max_hp += (money_laff - old_bean_amount)
	Util.get_player().stats.hp += (money_laff - old_bean_amount)
	grab_old_bean_amount()
	updating_money = false
